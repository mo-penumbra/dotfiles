# Generated by Powerlevel10k configuration wizard on 2025-12-01 at 15:17 UTC.
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    ...
    my_dir                  # current directory (shows hash name in parentheses)
    vcs                     # git status
  ...
  # Context format when in SSH without privileges: short name per host.
  case $HOST in
    devresourceintensivehost*) typeset -g POWERLEVEL9K_CONTEXT_{REMOTE,REMOTE_SUDO}_TEMPLATE='RIdev' ;;
    *macro*)                   typeset -g POWERLEVEL9K_CONTEXT_{REMOTE,REMOTE_SUDO}_TEMPLATE='Macro' ;;
    *engineering*)             typeset -g POWERLEVEL9K_CONTEXT_{REMOTE,REMOTE_SUDO}_TEMPLATE='Eng' ;;
    *)                         typeset -g POWERLEVEL9K_CONTEXT_{REMOTE,REMOTE_SUDO}_TEMPLATE='%1m' ;;  # fallback: first part of hostname
  esac
  ...
  # Custom dir segment: shows actual path with named directory in parentheses
  # e.g., ~/penumbra (~pig) only when at exact hashed directory (not subdirs)
  function prompt_my_dir() {
    local named="${(%):-%~}"       # Path with named dirs (e.g., ~pig or ~pig/foo)
    local actual="${PWD/#$HOME/~}" # Actual path with ~ for home

    # Only show hash if we're at the exact hashed directory (no subdirs)
    # i.e., named is ~something with no / in it
    if [[ "$named" == "~"?* && "$named" != *"/"* ]]; then
      p10k segment -f 31 -t "${actual} (${named})"
    else
      p10k segment -f 31 -t "${actual}"
    fi
  }
